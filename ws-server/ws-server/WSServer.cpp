...
void WSServer::packetCallback(void* userData, bool isVideoStream, unsigned char* buff, int size, long long pts, long long dts, long long duration) {
	...
	if (userData != nullptr) {
		...
		if (isVideoStream) {//暂时只发送视频数据
			...
			//写文件观察流是否正确,测试用
			//writeFile((char*)"./test.fmp4",buff,size);
		}
...
void WSServer::fmp4Callback(void* userData, unsigned char* buff, int size) {
	std::cout << "fmp4Callback  size=" << size << std::endl;
	//todo:此处还需要定义头附加到流数据首部，作为附加信息传递，例如携带流类型，流长度等
	if (userData != nullptr) {
		Userdata* userdata = (Userdata*)userData;
		...
		websocketpp::connection_hdl hdl = userdata->hdl;
			...
			//写文件观察流是否正确,测试用
			writeFile((char*)"./111.fmp4",buff,size);
...
void WSServer::handleTextMessage(websocketpp::connection_hdl hdl, server::message_ptr msg) {
	...
	do {
		...
		if (strcmp("open_url", req->valuestring) == 0) {
			...
			Fmp4CallbackFunction fun2 = std::bind(&WSServer::fmp4Callback, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
			takeStream->setFmp4Callback(fun2, userdata);
			...
			if (code == 0) {
				...
				char* mime=takeStream->getCodecMIME();
				if (mime) {
					cJSON_AddStringToObject(response, "mime", mime);
				}
			}
			else {
