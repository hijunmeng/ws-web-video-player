...
typedef std::function<void(void*, unsigned char*, int)> Fmp4CallbackFunction;
typedef void(*Fmp4Callback)(void* userData, unsigned char* buff, int size);
...
public:
	...
	int setFmp4Callback(Fmp4Callback  packetCallback, void* userData);
	int setFmp4Callback(Fmp4CallbackFunction packetCallback, void* userData);
	...
	int handleFmp4Data(uint8_t* buf, int buf_size);
...
private:
	...
	Fmp4Callback fmp4Callback;
	Fmp4CallbackFunction fmp4CallbackFunction;
	...
	int outVideoStreamIdx; //输出视频流下标
	int outAudioStreamIdx; //输出音频流下标
	uint8_t* pbBuffer;
